  ...
  useEffect(() => {
    const createAndFetchProject = async () => {
      try {
        ...
        formData.append("retail_unit", JSON.stringify([
          {
        ...
        formData.append("industrial_sites", JSON.stringify([
          {
        ...
        if (!data1.pid) {
          throw new Error(`PID not returned from Step 1: ${JSON.stringify(data1)}`);
        }
        ...
        formData2.append("retail_unit", JSON.stringify([
          {
            cstoresalespermonth: 0,
            foodsalespermonth: 0,
            carwashpermonth: 0,
        ...
        formData2.append("strategies", JSON.stringify([
          {
            num_mpds: 6,
            num_auto_diesel_hoses: 12,
            store_size: 6000,
        ...
        formData2.append("industrial_sites", JSON.stringify([
          {
            report_title: "skippers port",
            address: "2124 Marina Bay Drive, Kemah, TX",
            state: "TX",
    ...
    return (
      <div style={{ 
        ...
        <div>
          ...
          {projectId && <p style={{ fontSize: '14px', color: '#888' }}>Project ID: {projectId}</p>}
        </div>
      </div>
    );
  }
